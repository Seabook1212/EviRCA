...
This script orchestrates the workload workflow for a normal (baseline) run:
1. Trigger Locust load testing
2. Wait for load test duration
3. Collect Locust statistics

Collected workload data is organized into the dataset directory structure.
"""
...
EXP_ID = os.environ.get("EXP_ID", _random_exp_id())
RUN_TIME_SECONDS = _parse_duration_to_seconds(RUN_TIME)
WAIT_TIME_MINUTES = int((RUN_TIME_SECONDS + 59) // 60) + 1

# Base directories
SCRIPT_DIR = Path(__file__).parent.parent
DATASET_BASE = SCRIPT_DIR / "dataset_v2" / "normal_run" / EXP_ID
...
def generate_summary():
    ...
    with open(summary_path, "w") as f:
        ...
        f.write(f"  - Wait Time Minutes: {WAIT_TIME_MINUTES}\n")
        f.write(f"\n")
...
def main():
    ...
    orchestrator_start_time = datetime.now(timezone.utc)
    ...
    try:
        ...
        write_workload_metadata(workload_start_time, workload_end_time, orchestrator_start_time)
        ...
        duration = (end_time - orchestrator_start_time).total_seconds() / 60
